...
abstract class ReverseProxy(val backendHost: String, val proxyHost: String, val isDebug: Boolean = false) extends Controller {
	...
	def proxy[A, B](bodyParser: BodyParser[A], filter: ProxyFilter[A, B], interpreter: ProxyInterpreter[B])(implicit builder: ActionBuilder[Request]): Action[A] = builder.async(bodyParser) { request: Request[A] =>
		...
		if(isDebug) {
			debugLogging(request, proxyRequest, interpreterResponse, proxyResponse)
		}
